 ...
 *
 ...
 executed in the order of [5, 1, 4]  would be (0) + (5) + (5 + 1) = 11. The first query of duration 5, the second query of duration
 1 would have to wait 5 seconds (the duration of the first query) to be executed, and
...
public class MinimumWaitingTime {
  public int minimumWaitingTime(int[] queries) {
    ...
    for(int i = 0; i < queries.length - 1; i++) {
      sum = sum + queries[i];
      totalWaitTime = totalWaitTime + sum;
    }
